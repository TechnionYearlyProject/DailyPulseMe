/*
This is the router that routes every address to its destination
*/
import Vue from 'vue'
...
function checkAccount(push1, push2) {
    var self = this;
    Vue.http.get('https://webapp-180506135919.azurewebsites.net/users/isConnectedToGoogleCalendar', {
        headers: {
...
function checkToken(funcYes, funcNo) {
    var self = this;
    Vue.http.get('https://webapp-180506135919.azurewebsites.net/users/authenticateToken', {
        headers: {
            'Content-Type': 'application/json',
            'Authorization': localStorage.getItem('token')
        }
